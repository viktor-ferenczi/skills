...
- [`SDL_WINDOW_FULLSCREEN`](SDL_WindowFlags): fullscreen window at
  desktop resolution
- [`SDL_WINDOW_OPENGL`](SDL_WindowFlags): window usable with an OpenGL
  context
- [`SDL_WINDOW_HIDDEN`](SDL_WindowFlags): window is not visible
- [`SDL_WINDOW_BORDERLESS`](SDL_WindowFlags): no window decoration
- [`SDL_WINDOW_RESIZABLE`](SDL_WindowFlags): window can be resized
- [`SDL_WINDOW_MINIMIZED`](SDL_WindowFlags): window is minimized
- [`SDL_WINDOW_MAXIMIZED`](SDL_WindowFlags): window is maximized
- [`SDL_WINDOW_MOUSE_GRABBED`](SDL_WindowFlags): window has
  grabbed mouse focus
- [`SDL_WINDOW_INPUT_FOCUS`](SDL_WindowFlags): window has input
  focus
- [`SDL_WINDOW_MOUSE_FOCUS`](SDL_WindowFlags): window has mouse
  focus
- [`SDL_WINDOW_EXTERNAL`](SDL_WindowFlags): window not created by SDL
- [`SDL_WINDOW_MODAL`](SDL_WindowFlags): window is modal
- [`SDL_WINDOW_HIGH_PIXEL_DENSITY`](SDL_WindowFlags): window
  uses high pixel density back buffer if possible
- [`SDL_WINDOW_MOUSE_CAPTURE`](SDL_WindowFlags): window has mouse
  captured (unrelated to MOUSE_GRABBED)
- [`SDL_WINDOW_ALWAYS_ON_TOP`](SDL_WindowFlags): window should
  always be above others
- [`SDL_WINDOW_UTILITY`](SDL_WindowFlags): window should be treated as a
  utility window, not showing in the task bar and window list
- [`SDL_WINDOW_TOOLTIP`](SDL_WindowFlags): window should be treated as a
  tooltip and does not get mouse or keyboard focus, requires a parent
  window
- [`SDL_WINDOW_POPUP_MENU`](SDL_WindowFlags): window should be
  treated as a popup menu, requires a parent window
- [`SDL_WINDOW_KEYBOARD_GRABBED`](SDL_WindowFlags): window has
  grabbed keyboard input
- [`SDL_WINDOW_VULKAN`](SDL_WindowFlags): window usable with a Vulkan
  instance
- [`SDL_WINDOW_METAL`](SDL_WindowFlags): window usable with a Metal
  instance
- [`SDL_WINDOW_TRANSPARENT`](SDL_WindowFlags): window with
  transparent buffer
- [`SDL_WINDOW_NOT_FOCUSABLE`](SDL_WindowFlags): window should not
  be focusable

The [SDL_Window](SDL_Window) will be shown if
[SDL_WINDOW_HIDDEN](SDL_WindowFlags) is not set. If hidden at creation
...
[SDL_WINDOW_OPENGL](SDL_WindowFlags) or
[SDL_WINDOW_VULKAN](SDL_WindowFlags) flags, then the corresponding
...
If [SDL_WINDOW_VULKAN](SDL_WindowFlags) is specified and there isn't a
...
If [SDL_WINDOW_METAL](SDL_WindowFlags) is specified on an OS that does not
