...
Using [SDL_GPU_LOADOP_LOAD](SDL_GPULoadOp) before any contents have
been written to the texture subresource will result in undefined behavior.
[SDL_GPU_LOADOP_CLEAR](SDL_GPULoadOp) will set the contents of the
texture subresource to a single value before any rendering is performed.
It's fine to do an empty render pass using
[SDL_GPU_STOREOP_STORE](SDL_GPUStoreOp) to clear a texture, but in
...
- [SDL_PROP_GPU_TEXTURE_CREATE_D3D12_CLEAR_R_FLOAT](SDL_CreateGPUTexture): the red channel of the clear color
- [SDL_PROP_GPU_TEXTURE_CREATE_D3D12_CLEAR_G_FLOAT](SDL_CreateGPUTexture): the green channel of the clear color
- [SDL_PROP_GPU_TEXTURE_CREATE_D3D12_CLEAR_B_FLOAT](SDL_CreateGPUTexture): the blue channel of the clear color
- [SDL_PROP_GPU_TEXTURE_CREATE_D3D12_CLEAR_A_FLOAT](SDL_CreateGPUTexture): the alpha channel of the clear color

For the depth/stencil texture:

- [SDL_PROP_GPU_TEXTURE_CREATE_D3D12_CLEAR_DEPTH_FLOAT](SDL_CreateGPUTexture): the depth clear value
