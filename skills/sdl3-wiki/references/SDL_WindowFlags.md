...
When creating windows with [`SDL_WINDOW_RESIZABLE`](SDL_WindowFlags),
SDL will constrain resizable windows to the dimensions recommended by the
compositor to fit it within the usable desktop space, although some
compositors will do this automatically without intervention as well. Use
[`SDL_SetWindowResizable`](SDL_SetWindowResizable) after creation instead
